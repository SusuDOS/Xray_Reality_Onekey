...
#====================================================
# xray版本大于1.7.5
# 不支持xtls-rprx-direct/ xtls-rprx-origin流控
# 需要对流控部分进行修改

# 不支持ipv6的机子可能出问题，需要将nginx中的ipv6监听删除.
#====================================================
...
function system_check() {
  ...
  if [[ "${ID}" == "centos" && ${VERSION_ID} -ge 7 ]]; then
    ...
    INS="yum install -y"
    ${INS} wget
    wget -N -P /etc/yum.repos.d/ https://raw.githubusercontent.com/susudos/xray_reality_onekey/${github_branch}/basic/nginx.repo
...
function domain_check() {
  read -rp "请输入你的域名信息(eg: www.baidu.com):" domain
  ...
  echo -e "域名通过 DNS 解析的 IP 地址：${domain_ipv4} ${domain_ipv6}"
...
function update_sh() {
  ol_version=$(curl -L -s https://raw.githubusercontent.com/susudos/xray_reality_onekey/${github_branch}/install_tls.sh | grep "shell_version=" | head -1 | awk -F '=|"' '{print $3}')
  if [[ "$shell_version" != "$(echo -e "$shell_version\n$ol_version" | sort -rV | head -1)" ]]; then
    print_ok "存在新版本，是否更新 [Y/N]?"
    read -r update_confirm
    case $update_confirm in
    [yY][eE][sS] | [yY])
      wget -N --no-check-certificate https://raw.githubusercontent.com/susudos/xray_reality_onekey/${github_branch}/install_tls.sh
      print_ok "更新完成"
...
function configure_nginx() {
  ...
  cd /etc/nginx/conf.d/ && rm -f ${domain}.conf && wget -O ${domain}.conf https://raw.githubusercontent.com/susudos/xray_reality_onekey/${github_branch}/config/web.conf
...
function configure_xray() {
  cd /usr/local/etc/xray && rm -f config.json && wget -O config.json https://raw.githubusercontent.com/susudos/xray_reality_onekey/${github_branch}/config/pure_config.json
  modify_UUID
  modify_port
}

function configure_xray_ws() {
  cd /usr/local/etc/xray && rm -f config.json && wget -O config.json https://raw.githubusercontent.com/susudos/xray_reality_onekey/${github_branch}/config/xray_tls_ws.json
  modify_UUID
  modify_UUID_ws
  modify_port
  modify_fallback_ws
...
function configure_web() {
  ...
  case $webpage in
  [yY][eE][sS] | [yY])
    wget -O web.tar.gz https://raw.githubusercontent.com/susudos/xray_reality_onekey/main/basic/web.tar.gz
...
function vless_tls_link() {
  ...
  print_ok "vless://$UUID@$DOMAIN:$PORT?security=tls&flow=$FLOW#TLS_$DOMAIN"
  print_ok "-------------------------------------------------"
  print_ok "URL 二维码 (VLESS + TCP + TLS) （请在浏览器中访问）"
  print_ok "https://api.qrserver.com/v1/create-qr-code/?size=400x400&data=vless://$UUID@$DOMAIN:$PORT?security=tls%26flow=$FLOW%23TLS_$DOMAIN"
}

function vless_tls_information() {
  UUID=$(cat ${xray_conf_dir}/config.json | jq .inbounds[0].settings.clients[0].id | tr -d '"')
  PORT=$(cat ${xray_conf_dir}/config.json | jq .inbounds[0].port)
  FLOW=$(cat ${xray_conf_dir}/config.json | jq .inbounds[0].settings.clients[0].flow | tr -d '"')
  DOMAIN=$(cat ${domain_tmp_dir}/domain)
...
function ws_link() {
  ...
  print_ok "URL 链接 (VLESS + TCP + TLS)"
  print_ok "vless://$UUID@$DOMAIN:$PORT?security=tls#TLS_$DOMAIN"

  print_ok "URL 链接 (VLESS + WebSocket + TLS)"
  print_ok "vless://$UUID@$DOMAIN:$PORT?type=ws&security=tls&path=%2f${WS_PATH_WITHOUT_SLASH}%2f#WS_TLS_$DOMAIN"
  
  print_ok "-------------------------------------------------"
  print_ok "URL 二维码 (VLESS + TCP + TLS) （请在浏览器中访问）"
  print_ok "https://api.qrserver.com/v1/create-qr-code/?size=400x400&data=vless://$UUID@$DOMAIN:$PORT?security=tls%23TLS_$DOMAIN"


  print_ok "URL 二维码 (VLESS + WebSocket + TLS) （请在浏览器中访问）"
  print_ok "https://api.qrserver.com/v1/create-qr-code/?size=400x400&data=vless://$UUID@$DOMAIN:$PORT?type=ws%26security=tls%26path=%2f${WS_PATH_WITHOUT_SLASH}%2f%23WS_TLS_$DOMAIN"
}

function basic_information() {
  print_ok "VLESS+TCP+TLS 安装成功"
  vless_tls_information
  vless_tls_link
}

function basic_ws_information() {
  print_ok "VLESS+TCP+TLS+Nginx with WebSocket回落模式成功"
  ws_information
  print_ok "————————————————————————"
  vless_tls_information
  ws_link
...
function change2to1() {
  cd /usr/local/etc/xray && jq 'del(.inbounds[0].settings.fallbacks)' config.json > config.json_bak
  mv config.json_bak config.json
  restart_all  
}
...
menu() {
  ...
  echo -e "   *** 友情提示:证书申请一定时间内有数量限制 ***"
  echo -e "     *** 若证书申请失败,请更换二级域名即可 ***"
  ...
  echo -e "${Yellow}32.${Font} 删除回落端口 (国内转发一般不需要端口为一个网页,需要删除回落到nginx的端口)"
  ...
  14)
    if [[ ${shell_mode} == "ws" ]]; then
      read -rp "请输入路径(示例：/sus99dos/ 要求两侧都包含 /):" WS_PATH
      modify_fallback_ws
      modify_ws
      restart_all
    else
